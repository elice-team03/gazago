...
#### 윤보영
* 상품 상세 페이지
  * 상품정보 더보기 버튼 클릭 시 숨겨져 있던 상품 상세 이미지를 렌더링하는 기능 구현
  * 상품의 수량을 변경하고, 변경된 상품의 수량만큼 상품 가격을 변경

* 장바구니 페이지
  * localStorage를 사용해 장바구니 데이터를 관리함
  * 전체 선택 버튼 클릭 시 모든 상품이 선택되고 총 상품 가격을 전체 상품 가격으로 변경
  * 특정 상품을 선택/해제할 시 해당 상품의 가격 * 수량 만큼 전체 상품의 가격에서 추가/감소
  * 특정 상품의 수량을 변경할 시 해당 상품의 가격을 수량만큼 변경시킴
  * 선택된 상품을 삭제할 시 장바구니에서 삭제되고 localStorage 내 item에서도 삭제됨
  * 어떤 상품도 선택되어있지 않을 때 구매하기, 삭제하기 버튼 disabled 처리

* 주문 페이지
  * 신규 회원일 경우 신규 입력으로, 기존 회원이면서 이전에 배송 이력이 있는 경우 기존 배송지로 탭을 표시
  * 이전 배송지 목록 클릭 시 이전 배송기록 출력, pagination 구현
  * 우편번호 검색 버튼 클릭 시 daum 우편번호 검색 api 연동
  * 필수 항목 누락 또는 약관 미선택 시 경고 표시
  * stripe 결제 시스템 연동 화면 구현

* 주문내역 페이지
  * 현재 주문상태가 주문접수 또는 배송준비중일 때 배송지 변경, 주문 취소 버튼 활성화
  * 배송지 변경 버튼 클릭 시 text가 input으로 변경, 변경하기 클릭 시 변경된 배송지가 반영됨
  * 주문 취소 버튼 클릭 시 상품 상태를 주문 취소로 변경
* 관리자 페이지
